;; ==========================================================================
;; 1CMT_PARALLEL_FO_ZO_RATE_LIN
;; #37 of the one-compartment set
;;   Parallel absorption with first-order process and zero-order with rate
;;   Linear elimination
;; --------------------------------------------------------------------------
;; Structure
;;   DF x Dose     --(zero-order, R2)--> [Central (V)] --CL-->
...
$PROBLEM 1CMT parallel_fo_zo_rate | Linear elimination
...
$THETA  (0, 50.0)            ; 4 R2     zero-order rate into cmt 2 (mg/h)
$THETA  (0.001, 0.5, 0.999)  ; 5 DF     dose fraction (logit-scale IIV)

$OMEGA  0.09             ; 1 IIV CL
$OMEGA  0.09             ; 2 IIV V
$OMEGA  0.09             ; 3 IIV KA
$OMEGA  0 FIX            ; 4 IIV R2    (free to estimate if supported)
...
$TABLE   ID TIME AMT RATE EVID MDV CMT DV IPRED IRES IWRES CWRES
         PRED RES WRES
         ONEHEADER NOPRINT FILE=1cmt_parallel_fo_zo_rate_lin.tab
$TABLE   ID CL V KA R2 DF ETA1 ETA2 ETA3 ETA4 ETA5
         FIRSTONLY ONEHEADER NOPRINT FILE=1cmt_parallel_fo_zo_rate_lin.par
...
;        NOAPPEND ONEHEADER NOPRINT FILE=1cmt_parallel_fo_zo_rate_lin.tab
