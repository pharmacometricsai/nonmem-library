...
$DES
  ...
  IF(CD.GT.0) THEN
    ...
    EFF = XX/(1+XX)
  ENDIF
  DADT(2) = KIN*(1 - IMAX*EFF) - KOUT*A(2)
...
$TABLE   ID TIME AMT RATE EVID MDV CMT FLAG DV IPRED IRES IWRES CWRES
         ...
         ONEHEADER NOPRINT FILE=idr1_1cmt_iv_direct.tab
$TABLE   ID TIME CP EFF FACT RESP
         ONEHEADER NOPRINT FILE=idr1_1cmt_iv_direct.prof
$TABLE   ID CL V R0 KOUT IMAX IC50 HILL KIN ET1 ET2 ET3 ET4 ET5 ET6 ET7
         FIRSTONLY ONEHEADER NOPRINT FILE=idr1_1cmt_iv_direct.par
...
;        NOAPPEND ONEHEADER NOPRINT FILE=idr1_1cmt_iv_direct_sim.tab
