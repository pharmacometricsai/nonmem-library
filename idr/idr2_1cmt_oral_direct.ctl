...
; ---- typical values and between-subject variability ------------------
  ...
  LGTIMA = LOG(THETA(7)/(1-THETA(7))) + ETA(7)
  IMAX  = EXP(LGTIMA)/(1+EXP(LGTIMA))
  IC50  = THETA(8)*EXP(ETA(8))
...
$DES
  ...
  IF(CD.GT.0) THEN
    ...
    EFF = XX/(1+XX)
  ENDIF
  DADT(3) = KIN - KOUT*(1 - IMAX*EFF)*A(3)
...
$THETA  (0.001, 0.8, 0.999)  ; 7 IMAX  max. inhibition (logit-scale IIV)
$THETA  (0, 1.0)             ; 8 IC50  conc. for 50% of IMAX (mg/L)
...
$OMEGA  0 FIX            ; 7 IIV IMAX  (free it if the data support it)
$OMEGA  0.09             ; 8 IIV IC50
...
$TABLE   ID TIME AMT RATE EVID MDV CMT FLAG DV IPRED IRES IWRES CWRES
         ...
         SG4
         ONEHEADER NOPRINT FILE=idr2_1cmt_oral_direct.tab
$TABLE   ID TIME CP EFF FACT RESP
         ONEHEADER NOPRINT FILE=idr2_1cmt_oral_direct.prof
$TABLE   ID CL V KA F1 R0 KOUT IMAX IC50 HILL KIN ET1 ET2 ET3 ET4 ET5
         ET6 ET7 ET8 ET9
         FIRSTONLY ONEHEADER NOPRINT FILE=idr2_1cmt_oral_direct.par
...
;        NOAPPEND ONEHEADER NOPRINT FILE=idr2_1cmt_oral_direct_sim.tab
