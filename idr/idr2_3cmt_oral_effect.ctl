;; ==========================================================================
;; IDR2_3CMT_ORAL_EFFECT
;; Indirect response model, simultaneous PK/PD
;;   PD   : IDR model II - Inhibition of elimination
;;   PK   : 3-compartment, Oral, first-order absorption
;;   Link : Effect compartment (KE0), for counter-clockwise hysteresis
;; --------------------------------------------------------------------------
;; Structure
;;   Dose --> [Depot] --KA--> [Central (V2)]
;;   [Central (V2)] <--Q3--> [Periph.1 (V3)]
;;   [Central (V2)] <--Q4--> [Periph.2 (V4)]
;;   [Central (V2)] --CL--> eliminated
;;   plasma --KE0--> [Effect site (Ce)] --> drives the response
;;   KIN --> [Response (R)] --KOUT-->
;;   dR/dt = KIN - KOUT*(1 - IMAX*EFF)*R
;;   EFF = (CD/IC50)**HILL / (1 + (CD/IC50)**HILL)
;;   where CD = effect-site concentration Ce
;;
;; Behaviour
;;   The drug inhibits the first-order loss of the response.
;;   Response rises above baseline.  Because the drug acts on KOUT,
;;   the apparent turnover slows while drug is present - the return to
;;   baseline is dose-dependent, which is what separates model II from
;;   model III.
...
; compartment 5 = effect site (holds Ce directly)
; compartment 6 = response R (holds R directly)
...
; ---- typical values and between-subject variability ------------------
  ...
  KE0   = THETA(9)*EXP(ETA(9))
  R0    = THETA(10)*EXP(ETA(10))
  KOUT  = THETA(11)*EXP(ETA(11))
  LGTIMA = LOG(THETA(12)/(1-THETA(12))) + ETA(12)
  IMAX  = EXP(LGTIMA)/(1+EXP(LGTIMA))
  IC50  = THETA(13)*EXP(ETA(13))
  HILL  = THETA(14)*EXP(ETA(14))
...
$DES
  ...
  DIST2= Q4*(CP - CP2)
  DADT(1) = -KA*A(1)
  DADT(2) = KA*A(1) - CL*CP - DIST1 - DIST2
  DADT(3) =  DIST1
  DADT(4) =  DIST2
  DADT(5) = KE0*(CP - A(5))
  CD   = A(5)
  ...
  IF(CD.GT.0) THEN
    ...
    EFF = XX/(1+XX)
  ENDIF
  DADT(6) = KIN - KOUT*(1 - IMAX*EFF)*A(6)
...
$ERROR
  ...
  IF(CD.GT.0) THEN
    ...
    EFF = XX/(1+XX)
  ENDIF
  FACT = 1 - IMAX*EFF        ; factor applied to KOUT
  RESP = A(6)
  IF(FLAG.EQ.2) THEN
    IPRED = RESP                        ; response
...
$THETA  (0.001, 0.7, 0.999)  ; 8 F1    oral bioavailability (logit-scale IIV)
$THETA  (0, 0.5)             ; 9 KE0   effect-site equilibration rate (1/h)
$THETA  (0, 100.0)           ; 10 R0    baseline response (response units)
$THETA  (0, 0.1)             ; 11 KOUT  response turnover rate (1/h)
$THETA  (0.001, 0.8, 0.999)  ; 12 IMAX  max. inhibition (logit-scale IIV)
$THETA  (0, 1.0)             ; 13 IC50  conc. for 50% of IMAX (mg/L)
$THETA  (0.01, 1.0)          ; 14 HILL  Hill coefficient (fix to 1 for Emax)
...
$OMEGA  0.09             ; 9 IIV KE0
$OMEGA  0.09             ; 10 IIV R0
$OMEGA  0.09             ; 11 IIV KOUT
$OMEGA  0 FIX            ; 12 IIV IMAX  (free it if the data support it)
$OMEGA  0.09             ; 13 IIV IC50
$OMEGA  0 FIX            ; 14 IIV HILL  (free it if the data support it)
...
$TABLE   ID TIME AMT RATE EVID MDV CMT FLAG DV IPRED IRES IWRES
         CWRES PRED RES WRES
         ONEHEADER NOPRINT FILE=idr2_3cmt_oral_effect.tab
$TABLE   ID TIME CP CE EFF FACT RESP
         ONEHEADER NOPRINT FILE=idr2_3cmt_oral_effect.prof
$TABLE   ID CL V2 Q3 V3 Q4 V4 KA F1 KE0 R0 KOUT IMAX IC50 HILL KIN ETA1
         ETA2 ETA3 ETA4 ETA5 ETA6 ETA7 ETA8 ETA9 ETA10 ETA11 ETA12 ETA13
         ETA14
         FIRSTONLY ONEHEADER NOPRINT FILE=idr2_3cmt_oral_effect.par
...
;        NOAPPEND ONEHEADER NOPRINT FILE=idr2_3cmt_oral_effect.tab
