...
; ---- typical values and between-subject variability ------------------
  ...
  LGTIMA = LOG(THETA(5)/(1-THETA(5))) + ETA(5)
...
$DES
  ...
  IF(CD.GT.0) THEN
    ...
    EFF = XX/(1+XX)
  ENDIF
  DADT(2) = KIN - KOUT*(1 - IMAX*EFF)*A(2)
...
$THETA  (0.001, 0.8, 0.999)  ; 5 IMAX  max. inhibition (logit-scale IIV)
$THETA  (0, 1.0)             ; 6 IC50  conc. for 50% of IMAX (mg/L)
...
$OMEGA  0 FIX            ; 5 IIV IMAX  (free it if the data support it)
$OMEGA  0.09             ; 6 IIV IC50
...
$TABLE   ID TIME AMT RATE EVID MDV CMT FLAG DV IPRED IRES IWRES CWRES
         ...
         ONEHEADER NOPRINT FILE=idr2_1cmt_iv_direct.tab
$TABLE   ID TIME CP EFF FACT RESP
         ONEHEADER NOPRINT FILE=idr2_1cmt_iv_direct.prof
$TABLE   ID CL V R0 KOUT IMAX IC50 HILL KIN ET1 ET2 ET3 ET4 ET5 ET6 ET7
         FIRSTONLY ONEHEADER NOPRINT FILE=idr2_1cmt_iv_direct.par
...
;        NOAPPEND ONEHEADER NOPRINT FILE=idr2_1cmt_iv_direct_sim.tab
